...
import { Github, Linkedin, Footprints, GraduationCap, Award, BadgeAlert, Download, Mail, Home } from 'lucide-react'
      ...
      <div className="max-w-6xl w-full relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-12">
          ...
          <div className="relative shrink-0">
            <div className="w-40 h-40 md:w-56 md:h-56 lg:w-64 lg:h-64 border-4 border-black neo-shadow-lg overflow-hidden bg-yellow-300">
              <img 
          ...
          <div className="text-center lg:text-left">
            <div className="flex flex-wrap gap-2 justify-center lg:justify-start mb-4">
              {profile.titles.map((t) => (
                <div key={t} className="inline-block bg-yellow-300 border-4 border-black px-4 py-1 neo-shadow">
                  <span className="font-bold text-sm md:text-base">{t}</span>
                </div>
              ))}
            </div>
...
// Experience Section — timeline left, map right
...
// Hometown Section — map only, no timeline
function HometownSection() {
  return (
    <section className="py-12 md:py-20 px-4 relative overflow-hidden">
      <div className="absolute top-20 right-20 w-48 h-48 bg-cyan-200 rounded-full blur-blob animate-float-slow"></div>
      <div className="absolute bottom-10 left-10 w-32 h-32 bg-blue-200 rounded-full blur-blob-sm animate-pulse-glow"></div>
      <div className="absolute top-1/3 left-16 w-5 h-5 bg-cyan-400 border-2 border-black sparkle animate-float hidden md:block" style={{animationDelay: '0.5s'}}></div>
      <div className="absolute bottom-1/4 right-1/4 w-6 h-6 bg-blue-500 border-2 border-black sparkle-diamond animate-float hidden md:block" style={{animationDelay: '1s'}}></div>

      <div className="max-w-7xl mx-auto relative z-10">
        <SectionHeader color="cyan" icon={<Home size={28} />}>
          My Hometown
        </SectionHeader>
...
// Education Map Section — map left, education info right
function EducationMapSection() {
  const [mobileView, setMobileView] = useState<'list' | 'map'>('list')

  return (
    <section className="py-12 md:py-20 px-4 bg-purple-100 border-y-4 border-black relative overflow-hidden">
      <div className="absolute top-10 left-10 w-40 h-40 bg-purple-200 rounded-full blur-blob animate-pulse-glow"></div>
      <div className="absolute bottom-20 right-20 w-32 h-32 bg-pink-200 rounded-full blur-blob-sm"></div>
      <div className="absolute top-1/4 right-16 w-6 h-6 bg-purple-500 border-2 border-black sparkle animate-float hidden md:block" style={{animationDelay: '0.3s'}}></div>
      <div className="absolute bottom-1/3 left-20 w-5 h-5 bg-violet-400 border-2 border-black sparkle-diamond animate-float hidden md:block" style={{animationDelay: '1.2s'}}></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="bg-white border-4 border-black rounded-xl neo-shadow-lg overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row">
            ...
            {/* Education Info - Right side */}
            <div className={`md:w-2/5 lg:w-1/3 border-l-0 md:border-l-4 border-black ${mobileView === 'list' ? 'block' : 'hidden md:block'}`}>
              <div className="p-4 border-b-4 border-black bg-purple-50">
                <h3 className="font-black text-lg">My Education</h3>
              </div>
              <div className="max-h-125 md:max-h-150 overflow-y-auto p-4 space-y-4">
                {degrees.map((degree, index) => (
                  <div key={index} className="p-4 border-3 border-black rounded-lg bg-white neo-shadow-sm">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-10 h-10 bg-purple-400 border-2 border-black flex items-center justify-center shrink-0">
                        <GraduationCap size={18} />
                      </div>
                      <div>
                        <h4 className="font-black text-sm">{degree.degree} in {degree.fieldOfStudy}</h4>
                        <p className="text-xs text-gray-600">{degree.institution}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                      <span className="bg-yellow-200 border-2 border-black px-2 py-0.5 font-bold">
                        {degree.startYear} – {degree.endYear}
                      </span>
                      {degree.grade && (
                        <span className="bg-green-200 border-2 border-black px-2 py-0.5 font-bold">
                          {degree.grade}
                        </span>
                      )}
                    </div>
                    {degree.favoriteCourses && degree.favoriteCourses.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-bold text-gray-500 uppercase mb-1">Favorite Courses</p>
                        <div className="flex flex-wrap gap-1">
                          {degree.favoriteCourses.slice(0, 4).map((course, ci) => (
                            <span key={ci} className="px-2 py-0.5 bg-cyan-100 border border-black text-xs font-medium">
                              {course}
                            </span>
                          ))}
                          {degree.favoriteCourses.length > 4 && (
                            <span className="text-xs text-gray-500">+{degree.favoriteCourses.length - 4} more</span>
                          )}
                        </div>
                      </div>
                    )}
                    {degree.thesis && (
                      <div className="mt-2 p-2 bg-yellow-50 border-2 border-black">
                        <p className="text-xs font-bold text-gray-500 uppercase">Thesis</p>
                        <p className="text-xs text-gray-800">{degree.thesis.title}</p>
                      </div>
                    )}
                  </div>
                ))}
    ...
    <>
      <div className="grid-pattern md:py-10">
        <MainCanvasCard>
          ...
          {/* <HometownSection /> */}
          {degrees.length > 0 && <EducationMapSection />}
          {experiences.length > 0 && <ExperienceSection />}
